...
#   - 默认安装到 Claude Code 技能目录 | Installs to the Claude Code skills
#     directory by default: ~/.claude/skills
################################################################################
...
# Claude Code 在所有平台上使用 ~/.claude/skills | Claude Code uses ~/.claude/skills
# on every platform (macOS, Linux, Windows via Git Bash).
