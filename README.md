...
**自动安装 Automatic (macOS / Linux / Windows Git Bash):**

```bash
./install.sh --all    # 安装全部 install everything
./install.sh          # 交互式选择 interactive selection
```

技能安装到 Claude Code 的技能目录 Skills install to Claude Code's skills directory: `~/.claude/skills`

**手动安装 Manual:**

1. 将技能文件夹复制到 `~/.claude/skills/` | Copy a skill folder into `~/.claude/skills/`
...
| **输出 Output** | 视频 Video | **4–15s, 480p/720p/1080p/原生 native 4K, 含声音 with sound** |
...
├── CHANGELOG.md
├── install.sh                         # 安装脚本 Skill installer
└── skills/
    ├── 01-cinematic/
    │   ├── SKILL.md                   # English
    │   └── zh-CN/SKILL.md            # 简体中文
